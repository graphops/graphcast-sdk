...
struct IndexerData {
    ...
    pub graphNetwork: GraphNetworkJSON
}
...
pub async fn perform_indexer_query(
    graph_network_endpoint: String, variables: Variables,
) -> std::string::String  {
...
pub async fn query_indexer_stake(
    ...
    let variables: Variables = Variables{ address: indexer_address };
...
pub async fn query_indexer_allocations(
    ...
    let perform_indexer_query: IndexerResponse = serde_json::from_str(queried_result).unwrap(); 
    perform_indexer_query.data.indexer.allocations.into_iter().map(|a| a.subgraph_deployment.ipfs_hash).collect()
}

// Query indexer staking infomation, namely staked tokens and active allocations
pub async fn query_stake_minimum_requirement(
    graph_network_endpoint: String, indexer_address: String,
) -> String {
    let variables: Variables = Variables{ address: indexer_address };
    let queried_result = &perform_indexer_query(graph_network_endpoint, variables).await;

    let perform_indexer_query: IndexerResponse = serde_json::from_str(queried_result).unwrap(); 
    perform_indexer_query.data.graphNetwork.minimum_indexer_stake
}
