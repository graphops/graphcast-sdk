...
use colored::*;
use waku::{WakuPubSubTopic, WakuNodeHandle, Running, WakuNodeConfig, WakuLogLevel, waku_new, Multiaddr, ProtocolId};


pub fn setup_node_handle(graphcast_topics: Vec<Option<WakuPubSubTopic>>) -> WakuNodeHandle<Running> {
    ...
    const NODES: &[&str] = &[
        ...
        // block temp0
        // "/ip4/0.0.0.0/tcp/6001/p2p/16Uiu2HAmRYykxtTfTLL7ggJakG59H4MnTrmhGjwAAhcVjRDfbkCu"
    ];
    ...
    let node_handle = waku_new(Some(config.clone())).unwrap().start().unwrap();
    ...
    {
        let peerid = node_handle.add_peer(&address, ProtocolId::Relay).unwrap_or(String::from("Could not add peer"));
        node_handle.connect_peer_with_id(peerid, None).unwrap();
    }

    // Subscribe to desired topics
    for topic in graphcast_topics.clone() {
        node_handle.relay_subscribe(topic.clone()).expect("Could not subscribe to the topic");
        println!("PubSub peer readiness: {:#?} -> {:#}", topic.clone().unwrap().topic_name, node_handle.relay_enough_peers(topic).unwrap());
    }
    println!("listening to peers: {:#?}", node_handle.listen_addresses().unwrap());
