use waku::{WakuPubSubTopic, Encoding};

use crate::{client_registry::query_registry_indexer, client_network::{query_indexer_stake, query_indexer_allocations, query_stake_minimum_requirement}};
...
pub async fn generate_pubsub_topics(app_name: String, items: Vec<String>) -> Vec<Option<WakuPubSubTopic>> {
    items
        .into_iter()
        .map(|hash| {
