...
pub async fn perform_operator_indexer_query(
    registry_subgraph_endpoint: String, variables: Variables,
) -> std::string::String  {
...
pub async fn query_registry_indexer(
    ...
    let variables: Variables = Variables{ address: operator_address };
    let queried_result = &perform_operator_indexer_query(registry_subgraph_endpoint, variables).await;

    let perform_operator_indexer_query: GraphAccountResponse = serde_json::from_str(queried_result).unwrap(); 
    perform_operator_indexer_query.data.graph_account.gossip_operator_of.id
}
